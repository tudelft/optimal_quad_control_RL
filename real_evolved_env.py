...
params = symbols('k_wz, k_z, k_wx, k_x, k_wy, k_y, \
                 k_p1, k_p2, k_p3, k_p4, \
                 k_pd1, k_pd2, k_pd3, k_pd4, \
                 k_q1, k_q2, k_q3, k_q4, \
                 k_qd1, k_qd2, k_qd3, k_qd4, \
                 k_r1, k_r2, k_r3, k_r4, \
                 k_rd1, k_rd2, k_rd3, k_rd4, \
                 tau, k, w_min, w_max')

k_wz, k_z, k_wx, k_x, k_wy, k_y, \
k_p1, k_p2, k_p3, k_p4, \
k_pd1, k_pd2, k_pd3, k_pd4, \
k_q1, k_q2, k_q3, k_q4, \
k_qd1, k_qd2, k_qd3, k_qd4, \
...
vp0 = vb - np.dot(axes[0,:], vb)*axes[0,np.newaxis].T
vp1 = vb - np.dot(axes[1,:], vb)*axes[1,np.newaxis].T
vp2 = vb - np.dot(axes[2,:], vb)*axes[2,np.newaxis].T
vp3 = vb - np.dot(axes[3,:], vb)*axes[3,np.newaxis].T

vd0 = np.dot(axes[0,:], vb)
vd1 = np.dot(axes[1,:], vb)
vd2 = np.dot(axes[2,:], vb)
vd3 = np.dot(axes[3,:], vb)
...
fy = axes[0,1]*W1**2 + axes[1,1]*W2**2 + axes[2,1]*W3**2 + axes[3,1]*W4**2 \
    - 2*np.pi/0.127*(vd0*axes[0,1]*W1 + vd1*axes[1,1]*W2 + vd2*axes[2,1]*W3 + vd3*axes[3,1]*W4)

fz = axes[0,2]*W1**2 + axes[1,2]*W2**2 + axes[2,2]*W3**2 + axes[3,2]*W4**2 \
    - 2*np.pi/0.127*(vd0*axes[0,2]*W1+ vd1*axes[1,2]*W2 + vd2*axes[2,2]*W3 + vd3*axes[3,2]*W4)
    
dx = vp0[0,0]*W1 + vp1[0,0]*W2 + vp2[0,0]*W3 + vp3[0,0]*W4
dy = vp0[1,0]*W1 + vp1[1,0]*W2 + vp2[1,0]*W3 + vp3[1,0]*W4
dz = vp0[2,0]*W1 + vp1[2,0]*W2 + vp2[2,0]*W3 + vp3[2,0]*W4
...
My = k_q1*W1**2 + k_q2*W2**2 + k_q3*W3**2 + k_q4*W4**2 + k_qd1*d_W1 + k_qd2*d_W2 + k_qd3*d_W3 + k_qd4*d_W4
Mz = k_r1*W1**2 + k_r2*W2**2 + k_r3*W3**2 + k_r4*W4**2 + k_rd1*d_W1 + k_rd2*d_W2 + k_rd3*d_W3 + k_rd4*d_W4
...
k_rdn = (k_rd1 + k_rd2 + k_rd3 + k_rd4)/4 * (w_max)
